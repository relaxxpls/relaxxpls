...
\newcommand{\resumeItemAltLong}[3]{
  \begin{tabular*}{0.977\textwidth}{l@{\extracolsep{\fill}}r}
    \tiny $\bullet$ \hspace{2pt} \small #1 & \footnotesize \textit{#3} \\
    \hspace{3.75mm} \small #2 \\
  \end{tabular*}
}
...
\resumeSubHeadingListStart
  ...
  {\href{https://github.com/relaxxpls/Realest-Estates}{\textbf{Realest Estates}} $|$ \emph{Python, Django, PostgreSQL}}{(May. 2021 - Present)}
  \resumeItemListStart
    \resumeItem{Created a real estate website that allowed realtor users to add, modify and delete their property listings}
    \resumeItem{Built the backend server using the \textbf{Django} rest framework that communicated with a \textbf{Postgres} database}
  \resumeItemListEnd

  \resumeProjectHeadingAlt
  {\href{https://github.com/relaxxpls/CPPMatrixLib}{\textbf{CPPMatrixLib}} $|$ \emph{C\Plus\Plus, CMake, GTest, TravisCI}}{(Jan. 2021 -- Mar. 2021)}
  \resumeItemListStart
  ...
  \resumeItemListStart
    \resumeItem{Developed a smart suitcase that could follow its owner \& tested the algorithm through virtual simulation}
    \resumeItem{Implemented object detection via \textbf{OpenCV} \& utilised centroid tracking for mimicking target's motion}
    \resumeItem{Created a \textbf{Gazebo} simulation of the slave bot object tracking the master bot managed them using \textbf{ROS}}
  \resumeItemListEnd
\resumeSubHeadingListEnd
...
\resumeItemAltListStart
  \resumeItemAlt
  {Taught data structures and algorithms to UG freshers as a part of Summer of Science}
  {(Apr. 2021)}
  \resumeItemAlt
  ...
  {taught and tested the knowledge of Financial Statements, Capital Markets \& Ratio Analysis}
  {(Apr. 2020)}
	\resumeItemAlt
  ...
  {(2019 - 2020)}
  \resumeItemAlt
  {Directed \& edited a comedy short film that stood \textbf{3\textsuperscript{rd}} in the freshmen festival -- Freshiezza}
  {(Sep. 2019)}
	\resumeItemAlt
